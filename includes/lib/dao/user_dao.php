...
class UserDao {
    ...
    function searchMatching($conn, $valueObject) {
          ...
          if ($valueObject->getUserTypeId() != 0) {
              if ($first) { $first = false; }
              $sql = $sql."AND user_type_id = ".$valueObject->getUserTypeId()." ";
          }
    ...
    function singleQuery($conn, $sql, $valueObject) {
          ...
          if ($row = $conn->nextRow($result)) {
                   ...
                   $valueObject->setEmail($row[1]); 
                   $valueObject->setUsername($row[2]); 
                   $valueObject->setPassword($row[3]); 
                   $valueObject->setUserTypeId($row[4]); 
                   $valueObject->setName($row[5]); 
          } else {
